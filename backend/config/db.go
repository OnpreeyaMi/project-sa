...
func MockData() {
	...
	// --- Mock Customers ---
	customers := []entity.Customer{
		{FirstName: "สมชาย", LastName: "ใจดี", PhoneNumber: "0812345678", IsVerified: true},
		{FirstName: "สาวิตรี", LastName: "สุขใจ", PhoneNumber: "0898765432", IsVerified: true},
	}
	for _, c := range customers {
	...
	// --- Mock Addresses ---
	addresses := []entity.Address{
		{CustomerID: 1, AddressDetails: "123 หมู่ 5 ต.ในเมือง อ.เมือง จ.นครราชสีมา", IsDefault: true},
		{CustomerID: 2, AddressDetails: "456 หมู่ 8 ต.ท่าช้าง อ.เมือง จ.เชียงใหม่", IsDefault: true},
	}
	for _, a := range addresses {
	...
	// --- Mock Orders ---
	orders := []entity.Order{
		{CustomerID: 1, AddressID: 1, OrderNote: "ซักผ้าขาว/สี", OrderImage: "", /*อื่น ๆ*/},
		{CustomerID: 2, AddressID: 2, OrderNote: "ซักผ้าเด็ก", OrderImage: ""},
	}
	for _, o := range orders {
...
for _, m := range machines {
	DB.FirstOrCreate(
		...
		entity.Machine{
			...
			Machine_number: m.Machine_number, // ใช้หมายเลขเครื่องกันซ้ำ
		},
	)
}
