...
import React, { useState } from "react";
import EmpSidebar from "../../../component/layout/admin/AdminSidebar";
import { Card, Row, Typography, Button, Table, Space, Popconfirm, Modal, Input, Select, InputNumber, Form } from "antd";
...
const StockAdminPage: React.FC = () => {
  const [stockData, setStockData] = useState<StockItem[]>([
    { key: 1, name: "น้ำยาซักผ้า", quantity: 50, type: "detergent", lastUpdated: "2025-08-06T12:00:00Z", editor: "พนักงาน 1" },
    { key: 2, name: "น้ำยาปรับผ้านุ่ม", quantity: 30, type: "softener", lastUpdated: "2025-08-05T10:30:00Z", editor: "พนักงาน 2" },
    { key: 3, name: "น้ำยาอบผ้า", quantity: 20, type: "drying", lastUpdated: "2025-08-04T09:15:00Z", editor: "พนักงาน 3" },
    { key: 4, name: "น้ำยาปรับผ้านุ่ม", quantity: 0, type: "softener", lastUpdated: "2025-08-03T08:45:00Z", editor: "พนักงาน 4" },
  ]);
  ...
  const handleAddStock = (values: any) => {
    const newItem: StockItem = {
      key: stockData.length ? Math.max(...stockData.map(i => i.key)) + 1 : 1,
      name: values.name,
      type: values.type,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (quantity: number, record: StockItem) => (
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <Text>
            {quantity}{" "}
            {quantity < 10 && quantity > 0 && <Text type="danger">ใกล้หมด</Text>}
          ...
          <Button
            ...
            onClick={() => {
              const updatedData = stockData.map(item =>
                item.key === record.key
                  ? { ...item, quantity: item.quantity - 1, lastUpdated: new Date().toISOString(), editor: "currentUserName" }
                  : item
              );
              setStockData(updatedData);
            }}
    ...
    },
    {
      ...
      key: "lastUpdated",
      render: (lastUpdated: string) =>
        new Date(lastUpdated).toLocaleDateString("th-TH", { year: "numeric", month: "2-digit", day: "2-digit" }),
    },
    {
      title: "ผู้แก้ไข",
      dataIndex: "editor",
      ...
      render: (editor: string, record: StockItem) => (
        <Space>
          <Text>{editor}</Text>
          <Popconfirm title="คุณแน่ใจหรือไม่ที่จะลบสินค้า?" onConfirm={() => handleDelete(record.key)} okText="ใช่" cancelText="ยกเลิก">
  ...
  return (
    <EmpSidebar>
      <Title level={4} style={{ textAlign: "center", marginTop: 30, marginBottom: 30 }}>สินค้าที่มีอยู่ในสต็อก</Title>

      <Row gutter={[16, 16]} justify="center" style={{ marginBottom: 20 }}>
        <Card style={{ width: 400, height: 200, margin: 20, backgroundColor: "#F9FBFF" }}>
          <Text style={{ display: "block", textAlign: "center", fontSize: 50, fontWeight: "bold" }}>30</Text>
          <Text type="secondary" style={{ display: "block", textAlign: "center", fontSize: 16 }}>ยอดใช้งานวันนี้</Text>
        </Card>
        <Card style={{ width: 400, height: 200, margin: 20, backgroundColor: "#F9FBFF" }}>
          <Text style={{ display: "block", textAlign: "center", fontSize: 50, fontWeight: "bold" }}>20</Text>
          <Text type="secondary" style={{ display: "block", textAlign: "center", fontSize: 16 }}>ยอดใช้งานเดือนนี้</Text>
        </Card>
      </Row>

      <Row justify="center" style={{ marginBottom: 20 }}>
        <Space>
          <Button type="primary" style={{ height: 40, fontSize: 16 }} onClick={() => setIsModalVisible(true)}>เพิ่มรายการสินค้า</Button>
          <Button type="primary" style={{ height: 40, fontSize: 16 }}>ซื้อสินค้า</Button>
        </Space>
      </Row>

      <Card hoverable style={{ width: "100%", textAlign: "center", borderRadius: 8, background: "#F9FBFF" }}>
        <Table columns={columns} dataSource={stockData} pagination={{ pageSize: 5 }} style={{ margin: "0 auto", width: "80%" }} />
      </Card>

      <Modal title="เพิ่มรายการสินค้า" visible={isModalVisible} onCancel={() => setIsModalVisible(false)} footer={null}>
        <Form form={form} layout="vertical" onFinish={handleAddStock}>
          ...
          <Form.Item name="type" label="ประเภทสินค้า" rules={[{ required: true }]}>
            <Select>
          ...
          <Form.Item name="quantity" label="จำนวน" rules={[{ required: true, type: "number", min: 0 }]}>
            <InputNumber style={{ width: "100%" }} />
          </Form.Item>
          <Form.Item>
            <Button type="primary" htmlType="submit" style={{ width: "100%" }}>บันทึก</Button>
          </Form.Item>
        </Form>
      </Modal>
    </EmpSidebar>
